...
function parseUploadedDocumentText(rawTextOrBase64, mimeType) {
  ...
  const nameMatch = combinedText.match(/(?:1\.\s*)?(?:Name of (?:the )?Applicant|Beneficiary Name|Applicant Name|Name|नांव|नाव|नाम)\s*[:|-]?\s*([A-Z\s]{3,40})/i) || combinedText.match(/Shri\/Smt\s+([A-Z\s]{3,40})/i) || combinedText.match(/(AARAV SHARMA|RAJESH SURESH SHARMA|SUNITA RAMESH PATIL|SURESH SHARMA)/i);
  if (nameMatch) {
  ...
  let age = 28;
  let gender = "male";
  const ageMatch = combinedText.match(/(?:Male|Female|Transgender)\s*\/\s*(\d{1,2})\s*Years/i) || combinedText.match(/(\d{1,2})\s*Years/i) || combinedText.match(/(?:Age|वय|आयु)\s*[:|-]?\s*(\d{1,2})/i);
  ...
  let income = 25e4;
  const incomeMatch = combinedText.match(/(?:Applicant's Income|Assessed Annual Income|GROSS ANNUAL FAMILY INCOME|Annual Income|वार्षिक आय|उत्पन्न)[^Rs₹\d]{0,40}(?:Rs\.?|INR|₹|रु\.?)?\s*([\d,]+)/i) || combinedText.match(/(?:Rs\.?|INR|₹|रु\.?)\s*([\d,]+)(?:\/-|\s*per|\s*annual)?/i) || combinedText.match(/(250,?000|400,?000|100,?000|80,?000|50,?000)/);
  if (incomeMatch) {
  ...
  const distMatch = combinedText.match(/([A-Z\s]{3,20})\s+District/i) || combinedText.match(/(?:DISTRICT|जिल्हा|जिला)\s*[:|-]?\s*([A-Z\s]{3,20})/i);
